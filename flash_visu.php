...
if ( $row ) {
	...
	if ( ($fp = fopen($file, "r")) != 0 ) {
		...
		mysql_query("update flash_data set _hit = _hit + 1 where _IDinfos = '$IDinfos' ", $mysql_link);
		...
		$myrow   = ( $myres ) ? remove_magic_quotes(mysql_fetch_row($myres)) : 0 ;

		$mylinks = ( $myrow ) ? "[<a href=\"".myurlencode("index.php?$myrow[1]")."\">$myrow[0]</a>]" : "" ;

		do {
			$myrow    = remove_magic_quotes(mysql_fetch_row($myres));
			$mylinks .= ( $myrow ) ? ".[<a href=\"".myurlencode("index.php?$myrow[1]")."\">$myrow[0]</a>]" : "" ;
			} while ( $myrow );
...
//						print( read_author($article, $line, "index.php?item=19&IDinfos=$IDinfos&IDsubmenu=".@$_GET["IDsubmenu"]."&IDflash=$IDflash") );
					break;

				case "##ENDLOOP##" :
					while ( $article ) {
//						print( read_author($article, read_article($article, read_attachment($article, $temp, $href), true), $href) );
