...
if($generique == "off")
{
	$sql = "select * from ctn_items where _type = 1 and _nosemaine = ".date("W", js2PhpTime($st))."  and _IDcours = ".$IDx;
}
...
if(isset($_POST["submit"]))
{
	if($nbdev > 0)
	{
		if($generique == "on")
		{
			$update  = "update ctn_items set _texte = '".addslashes($texte)."' where _IDcours = $IDx and _nosemaine = $nosemaine and _type = 1";
		}
		else
		{
			$update  = "update ctn_items set _texte = '".addslashes($texte)."' where _IDcours = $IDx and _type = 1";
		}
	...
	else
	{
		if($generique == "on")
		{
			$update  = "insert into ctn_items values ('', '_IDctn', '$IDmat', '_ID' , '_IP', '', '1:00', 'chap', '".addslashes($texte)."', 'N', '', 'O', '1', '$IDx', '$nosemaine')";
		}
		else
		{
			$update  = "insert into ctn_items values ('', '_IDctn', '$IDmat', '_ID' , '_IP', '', '1:00', 'chap', '".addslashes($texte)."', 'N', '', 'O', '1', '$IDx', '$nosemaine')";
