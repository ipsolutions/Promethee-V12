...
function listCalendarByRange($sd, $ed){
  ...
  try{
	...
	switch ( $IDedt ) {
		case 1 :	// les salles
				...
				$sql .= "mat._titre '_titre', mat._color '_color', edt._IDx '_IDx', edt._semaine '_semaine', edt._ID '_ID' ";
				...
				$sql .= "and edt._nosemaine = '0' and edt._etat = '0' order by _jour, _heure";
			break;
		case 2 :	// le personnel
				$sql  = "select distinctrow edt._IDdata '_IDdata', edt._jour '_jour', edt._heure '_heure', edt._delais '_delais', ";
				$sql .= "mat._titre '_titre', mat._color '_color', edt._IDx '_IDx', edt._semaine '_semaine', edt._ID '_ID' ";
	...
	$rows    = ( $result ) ? mysql_fetch_row($result) : 0 ;
    ...
    while ($row = mysql_fetch_object($handle)) {
		if(intval($sem) == $row->_semaine || intval($row->_semaine) == 0 || $generique == "on") // verifie si semaine 1 ou 2 pour cours générique
		{
			...
			$queryuser .= "where _ID = ".$row->_ID;
			
			$resultuser = mysql_query($queryuser);
			$rowsuser    = ( $resultuser ) ? mysql_fetch_row($resultuser) : 0 ;
			
			if($IDedt == 1)
			{
				$titre_cours = "<b>".htmlentities($row->_titre)."</b> <br/>".strtoupper($rowsuser[0]);
			}
			else
			{
				$titre_cours = "<b>".htmlentities($row->_titre)."</b> <br/>".strtoupper($rowsuser[0]);
  ...
  /********** Tableau semaine patch SUPPRESSION **********/
	...
	switch ( $IDedt ) {
		case 1 :	// les salles
				$sql  = "select distinctrow edt._IDdata '_IDdata', edt._jour '_jour', edt._heure '_heure', edt._delais '_delais', ";
				$sql .= "mat._titre '_titre', mat._color '_color', edt._IDx '_IDx', edt._ID '_ID' ";
				$sql .= "from edt_data edt, campus_data mat, campus_classe classe ";
				$sql .= "where (edt._IDmat = mat._IDmat and mat._lang = '".$_SESSION["lang"]."') ";
				$sql .= "and edt._IDcentre = '".$IDcentre."' and edt._IDitem = '".$IDitem."' ";
				$sql .= "and edt._nosemaine = '".date("W", $sd)."' and edt._annee = '".date("Y", $sd)."' and edt._etat = '2' order by _jour, _heure";
			break;
		case 2 :	// le personnel
				$sql  = "select distinctrow edt._IDdata '_IDdata', edt._jour '_jour', edt._heure '_heure', edt._delais '_delais', ";
				$sql .= "mat._titre '_titre', mat._color '_color', edt._IDx '_IDx', edt._ID '_ID' ";
		...
		while ($row = mysql_fetch_object($handle)) {
			...
			// Classe si pas renseignée
			$queryclass  = "select _IDClass from edt_data where _IDdata = ".$row->_IDdata  ;
			
			$resultclass = mysql_query($queryclass);
			$rowsclass    = ( $resultclass ) ? mysql_fetch_row($resultclass) : 0 ;
			$titre_cours .= "<b>".htmlentities($row->_titre)."</b> <br/>".strtoupper($rowsclass[0]);
  ...
  /********** Tableau semaine patch AJOUT **********/
	...
	switch ( $IDedt ) {
		case 1 :	// les salles
				...
				$sql .= "where (edt._IDmat = mat._IDmat and mat._lang = '".$_SESSION["lang"]."') ";
				$sql .= "and edt._IDcentre = '".$IDcentre."' and edt._IDitem = '".$IDitem."' ";
				$sql .= "and edt._nosemaine = '".date("W", $sd)."' and edt._annee = '".date("Y", $sd)."' and edt._etat = '1' order by _jour, _heure";
			break;
		case 2 :	// le personnel
				$sql  = "select distinctrow edt._IDdata '_IDdata', edt._jour '_jour', edt._heure '_heure', edt._delais '_delais', ";
				$sql .= "mat._titre '_titre', mat._color '_color', edt._IDx '_IDx', edt._ID '_ID' ";
