...
; (function($) {
    ...
    $.fn.bcalendar = function(option) {
        ...
        var __ALLDAYEVENTTEMP = '<div class="rb-o ${eclass}" id="${id}" style="color:${color};"><div class="dhdV" style="display:none">${data}</div><div class="${extendClass} rb-m" style="background-color:${color}">${extendHTML}<div class="rb-i">${content}</div></div></div>';
        ...
        function BuildWT(ht, dayarrs, events, dMax) {
            ...
            for (var i = 0; i < dayarrs.length; i++) {
				...
				if($("#param_generique").val() == "off" || $("#param_generique").val() == "")
				{
        ...
        function BuildDayEvent(theme, e, index) {
			...
			var bgcolor1text = (e.event[12] > 0) ? theme[0] : theme[2] ;
            var p = { bdcolor: theme[0], bgcolor2: theme[0], bgcolor1: bgcolor1text, borderdev: borderdevtext, width: "100%", icon: "", title: "", data: "" };
        ...
        function dayshow(e, data) {
            ...
            if (data != null) {
                if (option.quickDeleteUrl != "" && data[8] == 1 && option.readonly != true) {
					...
					csbuddle  = '<div id="bbit-cs-buddle" style="background-color: #FFFFFF; z-index: 180; width: 400px;visibility:hidden; border: 1px solid '+ tc(data[7])[1] +'; border-radius: 4px" class="bubble">';
					csbuddle += '<div style="background-color: '+ tc(data[7])[2] +'"><div class="textbox-fill-wrapper"><div class="textbox-fill-mid"><div id="bbit-cs-what" title="'+ i18n.xgcalendar.click_to_detail + '" class="textbox-fill-div lk" style="cursor:pointer; color: #FFFFFF; font-weight: bold; font-size: 12px; text-decoration: none"></div><div id="bbit-cs-buddle-timeshow" style="position: absolute; top: 2px; right: 22px; color: #FFFFFF"></div></div></div><div id="bubbleClose2" class="bubble-closebutton"></div></div>';
					if($("#param_generique").val() == "off")
					{
						csbuddle += '<div style="padding: 4px"><div style="padding: 2px; margin-bottom: 5px; border: 1px dotted '+ tc(data[7])[1] +'; min-height: 30px"><div style="position: absolute; background-color: '+ tc(data[7])[2] +'; padding: 4px; right: 5px; margin-top: -2px; border-radius: 0 0 0 4px"><img src="images/edit_16.png" title="'+ i18n.xgcalendar.devoirs +'" /></div>'+txtdev+'</div>';
						csbuddle += '<div style="padding: 2px; margin-bottom: 5px; border: 1px dotted '+ tc(data[7])[1] +'; min-height: 30px"><div style="position: absolute; background-color: '+ tc(data[7])[2] +'; padding: 4px; right: 5px; margin-top: -2px; border-radius: 0 0 0 4px"><img src="images/contenu_16.png" title="'+ i18n.xgcalendar.contenu +'" /></div>'+txtcontenu+'</div>';
					}
					else
					{
						csbuddle += '<br /><br />';
					}
					csbuddle += '<input id="bbit-cs-id" type="hidden" value=""/>[ <span id="bbit-cs-delete" class="lk">'
                    	+ i18n.xgcalendar.i_delete + '</span> ]&nbsp;';
					if($("#param_generique").val() == "off")
					{
						csbuddle += '[ <span id="bbit-cs-adddev" class="lk">+ '+ i18n.xgcalendar.devoirs +'</span> ]&nbsp;[ <span id="bbit-cs-addcontenu" class="lk">+ '+ i18n.xgcalendar.contenu +'</span> ]&nbsp;';
					}
					csbuddle += '<SPAN id="bbit-cs-editLink" class="lk">'
                    ...
                    if (bud.length == 0) {
						...
						devbutton.click(function() {
							...
							OpenModelWindow(url,{ width: 600, height: 430, caption: i18n.xgcalendar.devoirs});
						});
						
						contenubutton.click(function() {
							var date = new Date(data[2]);
							// Ouverture fenêtre d'ajout de contenu du cours
							var url ="edt_addcontenu.php?IDx="+ data[0] +"&st="+ (data[2].getMonth()+1)+"/"+data[2].getDate()+"/"+data[2].getFullYear() +"&generique="+ $("#param_generique").val();
							OpenModelWindow(url,{ width: 600, height: 430, caption: i18n.xgcalendar.contenu});
						});
        ...
        function quickadd(start, end, isallday, pos) {
            ...
            if (buddle.length == 0) {
				...
				OpenModelWindow(url,{ width: 600, height: 400, caption: i18n.xgcalendar.new_event});
				//render();
