...
/*
 *		module   : Ldap2mysql.php
 *		projet   : implémentation de l'authentification LDAP
 *
 *		version  : 1.0
 *		auteur   : zetoutou
 *		creation : 25/09/06
...
class Ldap2Mysql
{
	...
	var $sMysqlTableToUpdate = 'user_id';
	...
	var $champ_motDePasse = '_passwd';
	var $champ_login = '_ident';
	//fin @promethee
	...
	function Ldap2Mysql($lang = 'fr_FR')
	{
		...
		define('CONFIG_DIR', MODULE_DIR . 'Authentification/config/');
		define('IMAGE_DIR',  MODULE_DIR . 'images/ldap2mysql/'); //
	...
	{
		$file = MODULE_DIR . 'Authentification/manual/' . 'manual_' . $this->lang . '.txt';
				...
				else
				{
					...
					switch ($obj->type->value)
					{
						...
						case 'openldap' :
							...
							//ajout des parametres rdn+pass pour la recherche du dn user
							$arr['rdn'] = $obj->login->value;
							$arr['rdn_pwd'] = $obj->pass->value;
							//fin modif zulu
							//
							break;
	...
	function importConfig_view()
	{
		...
		$aContent[] = '&raquo;&nbsp;' . $this->aParamLang['base']['libelle_1'] . ' <strong>LDAP</strong> <br/>' . $this->formSelectComponent('select_listeBase_ldap', $aLdapAllBases, array (
			'style="width:200px;"'
		));
		$aContent[] = '</div>';
		//liste des serveurs mysql	
		$aContent[] = '<div class="colonneDroite">';
		$aContent[] = '&raquo;&nbsp;' . $this->aParamLang['base']['libelle_1'] . ' <strong>MYSQL</strong> <br/>' . $this->formSelectComponent('select_listeBase_mysql', $aMysqlAllBases, array (
			'style="width:200px;"'
		));
		//
		...
		if (!empty ($aLdapFields) AND !empty ($aMysqlFields))
		{
			...
			$aContent[] = '<div class="colonneGauche">';
			$aContent[] = $this->aParamLang['base']['libelle_6'] . '<br/>' . $this->formSelectComponent('select_ldap_ListFields', $aLdapFields) . ' (<strong>' . count($aLdapFields) . '</strong>)&nbsp;';
			$aContent[] = '</div>';
			//
			//liste des champs mysql
			$aContent[] = '<div class="colonneDroite">';
			$aContent[] = $this->aParamLang['base']['libelle_7'] . '<br/>' . $this->formSelectComponent('select_mysql_ListFields', $aMysqlFields) . ' (<strong>' . count($aMysqlFields) . '</strong>)&nbsp;';
		...
		{
			$aContent[] = '<div class="row">';
			$aContent[] = SPACE2 . '<table id="ldap_import_correspondance">';
			//
	...
	*/
	...
	function ldap_import_view()
	{
		...
		if (!empty ($aoUser))
		{
			...
			$aContent[] = SPACE1 . '<div class="row">';
			//
			$aContent[] = SPACE2 . '<table class="ldap_import">';
			$aContent[] = SPACE3 . '<tr>';
		...
		if ($this->formVisible == true)
		{
			...
			foreach ($oBase as $propertie => $value)
			{
				...
				switch ($propertie)
				{
					...
					case 'type' :
						//
						$sComponent = $this->formSelectComponent('type', array (
	...
	function mysql_import_display()
	{
		...
		//liste des serveurs mysql
		$aContent[] = '<div class="row">';
		$aContent[] = '<div class ="divLeft">' . $this->aParamLang['base']['libelle_1'] . ' <strong>MYSQL</strong> : </div>';
		$aContent[] = '<div class ="divRight">';
	...
	*/
	...
	{
		//
		require_once 'msg/ldap2mysql/Lang.php';
		//
	...
	function display($sControlPanel)
	{
		...
		$aContent[] = '</div>';
		$aContent[] = '</form>';
		$aContent[] = '<div style="font-size:10px;font-style: italic;font-family:  \'georgia\',sans-serif;">' . $this->sDate_version . '</div>';
