...
function menu($idmenu, $sort)
{
	...
	while ( $row ) {
		...
		$row2    = ( $result2 ) ? remove_magic_quotes(mysql_fetch_row($result2)) : 0 ;
		...
		if ( $row[7] != 2 OR in_array($row[4], explode(" ", $myrow[0])) ) {
			...
			if ( strlen($row[2]) )
				...
				else
				{
					if($row2[0] > 0)
					{
						$mytext .= "<a href=\"$href\" title=\"$href\" $target style=\"margin-left: -1px; width: 302px\">". str_replace($keywords_search, $keywords_replace, $row[0]) ."</a>
						". makeSubmenu(-$row[4]) ."
						</li>";
					}
					else
					{
						$mytext .= "<a href=\"$href\" title=\"$href\" $target>". str_replace($keywords_search, $keywords_replace, $row[0]) ."</a>
						". makeSubmenu(-$row[4]) ."
						</li>";
					}
				}
...
function menu2($idmenu, $sort)
{
	...
	while ( $row ) {
		...
		if ( $_SESSION["CnxAdm"] == 255 OR $access == "visible" )
			$mytext .= "
				  ...
				  $icon <a href=\"index.php?$row[1]\" title=\"index.php?$row[1]\" style=\"$margin; margin-right: -1px; $padding\" >$icon2 $row[0]</a>
			      </li>";
	...
	while ( $row ) {
		...
		$link    = "<a href=\"index.php?item=0&amp;IDflash=$row[0]\" title=\"index.php?item=0&amp;IDflash=$row[0]\" style=\"margin-left: -1px; margin-right: -1px; padding-left: 5px\" ><img src=\"".$_SESSION["CfgPuce"]."\" title=\"\" alt=\"*\" /> ";
		$link   .= $msg->read($PAGE_FLASH, $row[1]);
		$link   .= "</a>";
				
		$mytext .= "<li style=\"margin-left: -1px; margin-right: -1px\">";
		$mytext .= "$link";
		$mytext .= "</li>";
...
function menu6($idmenu)
{
	...
	$Query  = "select _IDdata, _titre from link_data ";
	$Query .= "where _lang = '".$_SESSION["lang"]."' ";
	$Query .= "AND _IDparent = '0' AND _IDgroup = '0' ";
	...
	while ( $row ) {
		...
		$mytext .= "<li>";
		$mytext .= "<a href=\"index.php?item=22&amp;IDgroup=0&amp;IDparent=$row[0]\" style=\"margin-left: -1px; margin-right: -1px\"><img src=\"".$_SESSION["ROOTDIR"]."/images/link.png\" title=\"\" alt=\">\" /> $row[1]</a>";
		$mytext .= "</li>";

		$row = remove_magic_quotes(mysql_fetch_row($result));
		}

	// le portail internet de l'établissement
	...
	if ( $_SESSION["CfgWeb"] != "" ) {
		...
		$mytext .= "<li>";
		$mytext .= "<a href=\"$href\" title=\"$href\" $target style=\"margin-left: -1px; margin-right: -1px\"><img src=\"".$_SESSION["CfgPuce"]."\" title=\"\" alt=\"*\" /> ". $msg->read($PAGE_PORTAL) ."</a>";
		$mytext .= "</li>";
		}

	// lecture de la base de données des liens
	$Query  = "select link._titre, link._url from link ";
	$Query .= "where _lang = '".$_SESSION["lang"]."' ";
	$Query .= "AND _IDdata = '0' ";
	$Query .= "AND link._visible = 'O' ";
	$Query .= "AND link._IDgroup = '0' AND (link._IDgrprd & ".pow(2, $_SESSION["CnxGrp"] - 1).") ";
	$Query .= "order by link._titre" ;
	...
	while ( $row ) {
		...
		$href   = myurlencode("index.php?file=$row[1]");

		$mytext .= "<li>";
		$mytext .= "<a href=\"$href\" title=\"$href\" $target style=\"margin-left: -1px; margin-right: -1px\"> <img src=\"".$_SESSION["CfgPuce"]."\" title=\"\" alt=\"*\" />$row[0]</a>";
		$mytext .= "</li>";
...
function menu7($idmenu)
{
	...
	$mytext .= ( mysql_affected_rows($mysql_link) ) ? "<table class=\"width100\">" : "" ;
	...
	$mytext .= ( mysql_affected_rows($mysql_link) ) ? "</table>" : "" ;
	...
	if ( !mysql_affected_rows($mysql_link) OR $_SESSION["CnxSex"] == "A" )
...
function menu9($idmenu)
{
	...
	while ( $row ) {
		...
		$return = mysql_query("select _IDdata from humour_data where _ident = '$row[0]' limit 1", $mysql_link);
		...
		switch ( $myrow[0] ) {
			...
			case "doc" :
				...
				while ( $row ) {
					...
					$sqlret = mysql_query($query, $mysql_link);
					$data   = ( $sqlret ) ? remove_magic_quotes(mysql_fetch_row($sqlret)) : 0 ;
...
function menu18($idmenu)
{
	...
	$return  = mysql_query("select _email from user_id where _adm = '255' order by _ID limit 1", $mysql_link);
	$myrow   = ( $return ) ? mysql_fetch_row($return) : 0 ;
...
function setmenu($query)
{
	...
	$nbcount = ( $return ) ? mysql_affected_rows($mysql_link) : 0 ;
	...
	while ( $myrow ) {
		...
		if ( $_SESSION["CnxAdm"] == 255 OR $access == "visible" )
			switch ( $idmenu ) {
				...
				case "6" :
					...
					$mytext = ( $expand == "close" ) ? menu6($idmenu) : "" ;
					break;
		...
		if ( $mytext OR $expand == "open" ) {
			...
			{
				$mymenu = "<a href=\"index.php?cmde=setusermenu&amp;ident=$idmenu&amp;access=$expand\"><img src=\"".$_SESSION["ROOTDIR"]."/images/$expand.png\" style=\"margin-right: 5px;\" alt=\"img\"/>$mymenu</a>";
			}
