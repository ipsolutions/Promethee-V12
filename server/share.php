...
<?php
	...
	if ( $row ) {
		...
		$return = mysql_query("select distinctrow _ext from config_mime where _visible = 'O'", $mysql_link);
	...
	mysql_query("select _IDitem from p2p_items", $mysql_link);

	$nbfile = mysql_affected_rows($mysql_link);
	$link1  = "server.php?cmde=share&amp;lang=".$_SESSION["lang"];

	// total des établissements
	mysql_query("select _IDdata from p2p_data where _visible = 'O'", $mysql_link);
	...
	// choix de la langue
	$result  = mysql_query("select distinctrow _lang from p2p_items", $mysql_link);
	...
	if ( $nbelem ) {
		...
		while ( $row AND $i <= $MAXPAGE ) {
			...
			// établissement
			$return = mysql_query("select _ident, _url, _basedir from p2p_data where _IDdata = '$row[1]' limit 1", $mysql_link);
			$myrow  = ( $return ) ? remove_magic_quotes(mysql_fetch_row($return)) : 0 ;

			// licence
			$return = mysql_query("select _text from resource_license where _IDlicense = '$row[9]' AND _lang = '".$_SESSION["lang"]."' limit 1", $mysql_link);
