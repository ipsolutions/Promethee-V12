...
function showAbsent()
{
	...
	if ( $row ) {
		...
		$auth   = ( $res ) ? mysql_fetch_row($res) : 0 ;
		...
		while ( $row AND ($_SESSION["CnxID"] == $auth[0] OR ($auth[1] & pow(2, $_SESSION["CnxGrp"] - 1))) ) {
			...
			$return = ( getAccess($row[4]) == 1 )
				? mysql_query("select _ident from campus_classe where _IDclass = '$row[1]' limit 1", $mysql_link)
				: mysql_query("select _name, _fname, _sexe from user_id where _ID = '$row[1]' limit 1", $mysql_link) ;
